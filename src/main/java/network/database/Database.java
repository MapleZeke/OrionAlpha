...
/**
 * Our global Database operation handling. Pools connections and accesses raw database operations.
 *
 * @author Eric
 */
public class Database extends UnifiedDB {
   ...
   *
   * <p>This is the only real database that should be ever constructed within this JVM, using the
   * above info.
   *
  ...
  public static Database createInstance(
      String dbName, String serverName, String user, String passwd, int port) {
    if (instance == null) {
  ...
  /**
   * Initializes and/or retrieves the official instance of this JVM's Database. Used for all
   * UnifiedDB parameters throughout the source when accessing the Database or a DB Accessor.
   *
  ...
  public static int execute(Connection con, PreparedStatement propSet, Object... commands)
      throws SQLException {
    if (propSet != null) {
      ...
      if (result > 0) {
        ...
        if (rowsAffected == 0) {
          ...
          if (!query.contains("DELETE FROM") && !query.contains("INSERT INTO")) {
            ...
            else query = query.substring(query.indexOf("UPDATE"));
            ...
            if (newQuery.contains("WHERE"))
              rows =
                  newQuery.substring(
                      newQuery.indexOf("SET ") + "SET ".length(), newQuery.indexOf("WHERE "));
            else rows = newQuery.substring(newQuery.indexOf("SET ") + "SET ".length());
            // Construct an array of every front row
            String[] frontRows =
                rows.replaceAll(" = \\?, ", ", ").replaceAll(" = \\? ", ", ").split(", ");
            // Not all queries perform an UPDATE with a WHERE condition, allocate empty back rows
            String[] backRows = {};
            // If the query does contain a WHERE condition, parse the back rows (everything after
            // WHERE)
            if (newQuery.contains("WHERE")) {
              rows = newQuery.substring(newQuery.indexOf("WHERE ") + "WHERE ".length());
              backRows =
                  rows.replaceAll(" = \\? AND ", ", ").replaceAll(" = \\?", ", ").split(", ");
            }
            ...
            // Begin transforming the query - clear the rest of the string, transform to (Col1,
            // Col2, Col3)
