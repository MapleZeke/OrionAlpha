...
public class ShopDB {
  ...
  private static void rawGetItemLocker(User user) {
    try (Connection con = Database.getDB().poolConnection()) {
      try (PreparedStatement ps =
          con.prepareStatement("SELECT * FROM `itemlocker` WHERE `AccountID` = ?")) {
    ...
    try (Connection con = Database.getDB().poolConnection()) {
      try (PreparedStatement ps =
          con.prepareStatement(
              "UPDATE `inventorysize` SET `" + inventory + "` = ? WHERE `CharacterID` = ?")) {
  ...
  public static void rawInsertItemLocker(CashItemInfo cashItem) {
    try (Connection con = Database.getDB().poolConnection()) {
      try (PreparedStatement ps =
          con.prepareStatement(
              "INSERT INTO itemlocker (CashItemSN, AccountID, CharacterID, ItemID, CommodityID, Number, BuyCharacterName, ExpireDate) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ")) {
        Database.execute(
            con,
            ps,
            cashItem.getCashItemSN(),
            cashItem.getAccountID(),
            cashItem.getCharacterID(),
            cashItem.getItemID(),
            cashItem.getNumber(),
            cashItem.getBuyCharacterName(),
            cashItem.getDateExpire().fileTimeToLong());
      }
    } catch (SQLException ex) {
      ex.printStackTrace(System.err);
    }
  ...
  public static ReceivedGift rawLoadAccountByNameForGift(String rcvCharacterName) {
    ...
    try (Connection con = Database.getDB().poolConnection()) {
      try (PreparedStatement ps =
          con.prepareStatement(
              "SELECT * FROM `users` u JOIN `character` c ON u.AccountID = c.AccountID WHERE c.CharacterName = ?")) {
  ...
  public static void rawUpdateItemLocker(int characterID, List<CashItemInfo> cashItemInfo) {
    try (Connection con = Database.getDB().poolConnection()) {
      ...
      try (PreparedStatement ps =
          con.prepareStatement(
              "UPDATE `itemlocker` SET `AccountID` = ?, `CharacterID` = ?, `ItemID` = ?, `CommodityID` = ?, `Number` = ?, `BuyCharacterName` = ?, `ExpiredDate` = ? WHERE `CashItemSN` = ?")) {
        for (CashItemInfo cashInfo : cashItemInfo) {
          if (cashInfo != null) {
            ...
            Database.execute(
                con,
                ps,
                cashInfo.getAccountID(),
                cashInfo.getCharacterID(),
                cashInfo.getItemID(),
                cashInfo.getCommodityID(),
                cashInfo.getNumber(),
                cashInfo.getBuyCharacterName(),
                cashInfo.getDateExpire().fileTimeToLong(),
                cashInfo.getCashItemSN());
          }
        }
      }
  ...
  public static void rawUpdateNexonCash(int accountID, int nexonCash) {
    try (Connection con = Database.getDB().poolConnection()) {
      try (PreparedStatement ps =
          con.prepareStatement("UPDATE `users` SET `NexonCash` = ? WHERE `AccountID` = ?")) {
