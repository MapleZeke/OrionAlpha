...
public class GameDB {
    ...
    try (Connection con = Database.getDB().poolConnection()) {
      try (PreparedStatement ps =
          con.prepareStatement(
              "SELECT `TargetID`, `LastGivePopularity` FROM `givepopularity` WHERE `CharacterID` = ? AND DATEDIFF(NOW(), `LastGivePopularity`) < 30")) {
      ...
      if (retCode == GivePopularityRes.Success) {
        try (PreparedStatement ps =
            con.prepareStatement(
                "INSERT INTO `givepopularity` (`CharacterID`, `TargetID`) VALUES (?, ?)")) {
  ...
  public static void rawLoadAccount(
      int characterID,
      Pointer<Integer> accountID,
      Pointer<String> nexonClubID,
      Pointer<Integer> gradeCode) {
    try (Connection con = Database.getDB().poolConnection()) {
      try (PreparedStatement ps =
          con.prepareStatement(
              "SELECT * FROM `users` u JOIN `character` c ON u.AccountID = c.AccountID WHERE c.CharacterID = ?")) {
        ps.setInt(1, characterID);
        try (ResultSet rs = ps.executeQuery()) {
          if (rs.next()) {
  ...
  public static CharacterData rawLoadCharacter(int characterID) {
    ...
    try (Connection con = Database.getDB().poolConnection()) {
      ...
      // Load Skills
      try (PreparedStatement ps =
          con.prepareStatement("SELECT * FROM `skillrecord` WHERE `CharacterID` = ?")) {
        ps.setInt(1, characterID);
        try (ResultSet rs = ps.executeQuery()) {
      ...
      // Load Quests
      try (PreparedStatement ps =
          con.prepareStatement("SELECT * FROM `questperform` WHERE `CharacterID` = ?")) {
        ps.setInt(1, characterID);
        try (ResultSet rs = ps.executeQuery()) {
      ...
      if (updateEquip) {
        CommonDB.rawUpdateItemEquip(
            characterID, cd.getEquipped(), cd.getEquipped2(), cd.getItemSlot(ItemType.Equip));
      }
  ...
  public static int rawLoadReward(int templateID, List<RewardInfo> rewardInfo) {
    ...
    try (Connection con = Database.getDB().poolConnection()) {
      try (PreparedStatement ps =
          con.prepareStatement("SELECT * FROM `reward` WHERE `TemplateID` = ?")) {
  ...
  public static void rawSaveCharacter(CharacterStat cs) {
    try (Connection con = Database.getDB().poolConnection()) {
      try (PreparedStatement ps =
          con.prepareStatement(
              "UPDATE `character` SET `CharacterName` = ?, `Gender` = ?, `Skin` = ?, `Face` = ?, `Hair` = ?, `Level` = ?, `Job` = ?, `STR` = ?, `DEX` = ?, `INT` = ?, `LUK` = ?, `HP` = ?, `MP` = ?, `MaxHP` = ?, `MaxMP` = ?, `AP` = ?, `SP` = ?, `EXP` = ?, `POP` = ?, `Money` = ?, `Map` = ?, `Portal` = ? WHERE `CharacterID` = ?")) {
        Database.execute(
            con,
            ps,
            cs.getName(),
  ...
  public static void rawSaveSkillRecord(int characterID, Map<Integer, Integer> skillRecord) {
    try (Connection con = Database.getDB().poolConnection()) {
      try (PreparedStatement ps =
          con.prepareStatement(
              "UPDATE `skillrecord` SET `Info` = ? WHERE `CharacterID` = ? AND `SkillID` = ?")) {
  ...
  public static void rawSetInventorySize(int characterID, List<Integer> inventorySize) {
    try (Connection con = Database.getDB().poolConnection()) {
      try (PreparedStatement ps =
          con.prepareStatement(
              "UPDATE `inventorysize` SET `EquipCount` = ?, `ConsumeCount` = ?, `InstallCount` = ?, `EtcCount` = ? WHERE `CharacterID` = ?")) {
        Database.execute(
            con,
            ps,
            inventorySize.get(0),
