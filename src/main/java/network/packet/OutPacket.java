...
public class OutPacket {
  ...
  /**
   * Initializes the packet with the given packet header. If the <code>type</code> is equal to
   * <b>Integer.MAX_VALUE</b>, then the packet header is ignored.
   *
  ...
  /**
   * Initializes the byte header of this packet. In KMS Alpha, packet headers are written as bytes
   * and not shorts.
   *
  ...
  /**
   * Using the raw <code>seqBase</code> and the crypter's <code>seqKey</code>, this method will
   * perform bitwise operation which will result in encoding the final sequence base containing both
   * the version of the game, and the client's current sequence.
   *
   * <p>This method is designed to encode a sequence that determines if the packet is both correct,
   * and that the server's version matches the client's version.
   *
  ...
  /**
   * Encodes a MapleStory String. A MapleStory String is encoded as follows: [01 00] -> The size of
   * the string [00] -> The <code>OUT_ENCODING</code> character(s) to encode
   *
  ...
  /**
   * Encodes a fixed-length string. A fixed-length is a length that the MapleStory client requires.
   * If the string is shorter than <code>size</code>, then it will append null-termination
   * characters to fill the remaining length.
   *
  ...
  /**
   * Encodes an array of bytes. The amount of bytes written in total length will be equivalent to
   ...
   *
   * <p>While this should never actually be relied on, this will help with packets that contain
   * unknown lengths or are crashing from lack of data. Using this should help identify the
   * remaining bytes and how many are missing.
   *
