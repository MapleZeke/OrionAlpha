...
public class InPacket {
  ...
  /**
   * Initializes this InPacket with the proper sequencing and data length. After the initial state,
   * this continues to append the remaining buffer's raw data into this packet's buffer to be
   * decrypted.
   *
  ...
  public int appendBuffer(ByteBuf buff, Pointer<Integer> lastState) {
    ...
    if (lastState != null) lastState.set(state);
  ...
  /**
   * Copies the remaining data from this buffer and writes it to an OutPacket buffer to be encoded.
   *
  ...
  /**
   * Decodes a 4-byte float. This is not typically used in MapleStory.
   *
  ...
  /**
   * Using the raw sequencing of this packet and the <code>seqKey</code> given, this method will
   * perform bitwise operation which will result in equaling the sequence base (a.k.a the version)
   * of the game.
   *
   * <p>This method is designed to validate if the packet is both correct, and that the server's
   * version matches the client's version.
   *
  ...
  /**
   * Decodes a string of characters with a given size. MapleStory Strings are decoded as follows:
   * [01 00] -> Size of the string [00] -> The character(s) in <code>IN_ENCODING</code>
   *
  ...
  /**
   * Constructs a readable string displaying the original bytes of the packet. This will print both
   * the overall buffer from start to finish, as well as the remaining bytes of the buffer from the
   * current offset to the end of the buffer.
   *
  ...
  public String dumpString() {
    ...
    if (data.length - recvBuff.readerIndex() > 0) {
      ...
      System.arraycopy(
          data, recvBuff.readerIndex(), remain, 0, data.length - recvBuff.readerIndex());
  ...
  /**
   * Appends an entire buffer worth of bytes into this packet's buffer. This will only append the
   * data to the buffer if the new size exceeds the current length of this packet. In addition, this
   * will increment the length of this packet by <code>size</code>.
   *
  ...
  /**
   * Strictly assigns the overall length of this packet's buffer. NOTE: This method should only ever
   * be used in regards to JVM->JVM packets. For instance, we use this for Game->Login and
   * Login->Game networking.
   *
