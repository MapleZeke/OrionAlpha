...
public class ClientSocket extends SimpleChannelInboundHandler {
  ...
  public void channelRead0(ChannelHandlerContext ctx, Object msg) throws Exception {
    ...
    try {
      ...
      if (packet.getOffset() == 4) super.channelRead(ctx, msg);
    }
  }
  ...
  public void onAliveAck() {
    ...
    if (this.aliveReqSent > 0
        && (cur - this.aliveReqSent) <= 60000 * (this.firstAliveAck ? 8 : 3)) {
      this.aliveReqSent = 0;
  ...
  public void onClose() {
    ...
    if (this.loginState < 7 || this.loginState >= 12) this.accountID = 0;
    this.loginState = 13;
  ...
  public void onCreateNewCharacter(InPacket packet) {
    // [04 00 45 72 69 63] [21 4E 00 00] [4B 75 00 00] [8A DE 0F 00] [A2 2C 10 00] [85 5B 10 00] [F0
    // DD 13 00] [07 00 00 00] [06 00 00 00] [06 00 00 00] [06 00 00 00]
    if (this.loginState != 8) {
      postClose();
      return;
    }
    ...
    if (totStat > 25) ret = false;

    // Check Character Name
    if (!LoginApp.getInstance().checkCharName(charName, true)) ret = false;
    ...
    if (this.loginState == 8) {
      ...
      if (world != null) {
        ...
        if (ch != null) {
          ...
          sendPacket(
              LoginPacket.onSelectCharacterResult(
                  1, Utilities.netIPToInt32(ch.getAddr()), ch.getPort(), this.characterID),
              false);
        }
      }
    } else {
  ...
  public void onSelectWorld(InPacket packet) {
    ...
    if (pWorld != null) {
      ...
      Logger.logError(
          "User %s attempting to connect to offline world %d", this.nexonClubID, this.worldID);
    }
  }
  ...
  private void processPacket(InPacket packet) {
    ...
    if (OrionConfig.LOG_PACKETS) {
      Logger.logReport(
          "[Packet Logger] [0x%s]: %s",
          Integer.toHexString(type).toUpperCase(), packet.dumpString());
    }
  ...
  public void sendPacket(OutPacket packet, boolean force) {
    if (packet.getOffset() >= 0x32000) {
      Logger.logError(
          "SendPacket size is more than 200KB. Dump: %s IP: %s",
          packet.dumpString(), getSocketRemoteIP());
      return;
    }
    this.lockSend.lock();
    try {
