...
public class ExpAccessor {
   ...
   */
  public static boolean tryProcessLevelUp(
      CharacterData cd, BasicStat bs, int inc, Pointer<Boolean> reachMaxLv) {
  ...
  /**
   * Used to decrease a player's EXP if they die. This function is only ever called if the user does
   * not have a Safety Charm/etc.
   *
    ...
    if (job != 0) {
      if (level >= MAX_LEVEL) {
        ...
        if (town) {
          ...
          if (JobAccessor.getJobCategory(job) == JobCategory.Archer) rate = 0.08;
          else rate = 0.2;
        ...
        c.getCharacterStat()
            .setEXP(
                Math.min(Math.max(0, c.getCharacterStat().getEXP() - (int) (exp * rate)), exp - 1));
      }
    }
  }
  ...
  /**
   * Does the reverse. Instead of giving the EXP, it will find the player's level. Note that this
   * specifically only checks for Explorers.
   *
