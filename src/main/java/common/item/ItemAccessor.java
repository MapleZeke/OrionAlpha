...
public class ItemAccessor {
  ...
  public static boolean isTreatSingly(ItemSlotBase p) {
    ...
    return !isBundleTypeIndex(ti)
        || isRechargeableItem(p.getItemID())
        || FileTime.compareFileTime(p.getDateExpire(), FileTime.END) < 0;
  }
  ...
  public static void getRealEquip(
      CharacterData c, List<ItemSlotBase> realEquip, int excl1, int excl2) {
    ...
    while (pos <= BodyPart.BP_Count) {
      for (pos = 1; pos <= BodyPart.BP_Count; pos++) {
        ...
        if (equip != null) {
          ...
          if (!ItemInfo.isAbleToEquip(
              gender, level, job, totSTR, totDEX, totINT, totLUK, pop, equip.getItemID())) {
            incSTR -= equip.iSTR;
            incDEX -= equip.iDEX;
            incINT -= equip.iINT;
            incLUK -= equip.iLUK;
  ...
  public static int getBodyPartFromItem(
      int itemID, int gender, Pointer<Integer> bodyPart, boolean all) {
