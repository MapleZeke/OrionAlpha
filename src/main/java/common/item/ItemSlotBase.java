...
public abstract class ItemSlotBase implements Comparable<ItemSlotBase> {
  ...
  /**
   * Constructs (or, creates) a new Item object. This method is designed to create a "placeholder"
   * item where nothing has been assigned nor being used, as it is only to be used for Cloning an
   * item, or Decoding a item from a remote server (Center/Login/etc)
   *
  ...
  /**
   * Encodes, or writes, the current item's information into the OutPacket buffer.
   *
  ...
  /**
   * Determines if the item has a valid Cash Item Serial Number that is greater than zero, marking
   * this item a Cash Item.
   *
  ...
  /**
   * An in-depth comparison of two items. If the two items are equips, it will continue to compare
   * each individual equipment stats to determine equality.
   *
   * @param src The item to compare
   * @return If the two items are the same
   */
  public boolean isSameItem(ItemSlotBase src) {
    if (this.itemID != src.itemID
        || this.cashItemSN != src.cashItemSN
        || this.dateExpire != src.dateExpire) {
      return false;
    } else {
  ...
  /**
   * Encodes, or writes, the raw item information, such as the ItemID, CashItemSN, and expiration.
   *
   * <p>A note about this packet is that each inherited class will override this function to encode
  ...
  /**
   * The quantity of the item. This variable should only ever be changed for items whose type is of
   * Bundle. All others should remain 1.
   *
   * @return
   */
  ...
  /**
   * A base will include a generated CashItemSN, but regular SN's are generated/read per class. This
   * will get the item's regular tracking SN.
   *
   * @return
   */
  ...
  /**
   * While the Center server determines the base Type Index by the ItemID the base holds, each
   * inherited class will return their proper Item Slot Type value.
   *
   * @return
   */
  ...
  /**
   * Nexon has the ability to construct a clone by using an inherited GetDataSize block and copying
   * the memory to the new base class. We can't do that so we'll override it per class.
   *
   * @return
   */
