...
public class CharacterData {
  ...
  static final int BodyPartCount = BodyPart.BP_Count, ItemTypeCount = ItemType.NO;
  ...
  public void backupItemSlot(
      List<List<ItemSlotBase>> backupItem, List<List<Integer>> backupItemTrading) {
  ...
  public ItemSlotBase getItem(byte ti, int pos) {
    if (ti >= ItemType.Equip && ti <= ItemType.Etc) {
      if (ti == ItemType.Equip) {
        if (pos != 0
            && (pos >= -BodyPartCount || pos < -BodyPartCount)
            && pos <= getItemSlotCount(ItemType.Equip)
            && pos >= -BodyPartCount - BodyPartCount) {
  ...
  public void restoreItemSlot(
      List<List<ItemSlotBase>> backup, List<List<Integer>> backupItemTrading) {
    for (int ti = ItemType.Equip; ti <= ItemType.Etc; ti++) {
  ...
  public static boolean CheckAdult(int nSSN1, int nSSN2, int nCriticalYear) {
    ...
    if (nSSN2 / 1000000 != 1
        && nSSN2 / 1000000 != 2
        && nSSN2 / 1000000 != 5
        && nSSN2 / 1000000 != 6) nBirthYear = nSSN1 / 10000 + 2000;
    else nBirthYear = nSSN1 / 10000 + 1900;
