...
public class GameApp {
  ...
  private void initializeDB() {
    try (JsonReader reader = Json.createReader(new FileReader("Database.img"))) {
      ...
      Database.createInstance(
          dbName, dbSource, dbInfo[0], dbInfo.length == 1 ? "" : dbInfo[1], dbPort);
  ...
  private void initializeGameData() {
    ...
    ItemInfo.load();
    Logger.logReport(
        "Loaded Item Info in " + ((System.currentTimeMillis() - time) / 1000.0) + " seconds.");

    // Load Skills
    time = System.currentTimeMillis();
    SkillInfo.getInstance();
    Logger.logReport(
        "Loaded Skill Info in " + ((System.currentTimeMillis() - time) / 1000.0) + " seconds.");

    // Load Mobs
    time = System.currentTimeMillis();
    MobTemplate.load(true);
    Logger.logReport(
        "Loaded Mob Attributes in " + ((System.currentTimeMillis() - time) / 1000.0) + " seconds.");

    // Load Npcs
    time = System.currentTimeMillis();
    NpcTemplate.load(true);
    Logger.logReport(
        "Loaded Npc Attributes in " + ((System.currentTimeMillis() - time) / 1000.0) + " seconds.");

    // Load Maps
    time = System.currentTimeMillis();
    FieldMan.init(getChannels().size());
    Logger.logReport(
        "Loaded map (field) data from map files in "
  ...
  public void start() {
    ...
    createAcceptor();
    Logger.logReport(
        "The Game Server has been initialized in "
            + ((System.currentTimeMillis() - serverStartTime) / 1000.0)
            + " seconds.");
  }
