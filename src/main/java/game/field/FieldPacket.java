...
public class FieldPacket {
   ...
   *
   * <p>In later versions of the game Nexon has types that display messages. However, this seems to
   * just act as a "onExclRequest" type packet, which means it just resets the client's request and
  ...
  /**
   * The weird message packet - displays yellow in chat the Name:Message. Oh, and it also renders
   * the same Name:Message above the chat window.
   *
   * <p>Since groups (buddy, party, guild, etc.) don't exist yet, and no types for BroadcastMsg
   * exist for megaphones, my best guess is that Nexon uses this packet for megaphones in this
   * client.
   *
  ...
  /**
   * The OnWhisper Packet for /find and Whispering purposes. Refer to WhisperFlags for the list of
   * results that are used within this packet.
   *
   ...
   */
  public static OutPacket onWhisper(
      byte flag, String find, String receiver, String msg, int location, boolean success) {
