...
public class WvsPhysicalSpace2D {
  ...
  public LadderOrRope findLadderOrRope(int x, int y, int dx, int dy) {
    for (LadderOrRope p : ladderOrRope) {
      if (p.x >= x - dx && p.x <= x + dx && p.y1 - dy <= y && dy + p.y2 >= y) return p;
    }
    return null;
  }
  ...
  public StaticFoothold getFootholdClosest(
      Field field, int x, int y, Pointer<Integer> pcx, Pointer<Integer> pcy, int hitx) {
        ...
        if (hitx <= x || x2 <= x) {
          if (pfh.getY1() >= y - 100) {
            if (pfh.getY2() >= y - 100) {
              ...
              if (hitx <= x) {
                if (hitx >= x) y0 = (pfh.getY2() + pfh.getY1()) / 2;
                else y0 = pfh.getY2();
              } else {
                y0 = pfh.getY1();
              }
              ...
              if (dist < minimum) {
                if (x > x1 && (x >= x2 || x - (x1 + x2) / 2 >= 0)) x1 = x2;
                x1 = Math.min(Math.max(x1, mbr.left + 10), mbr.right - 10);
                int y1 =
                    pfh.getY1()
                        + (((pfh.getY2() - pfh.getY1()) * (x1 - pfh.getX1())) / (x2 - pfh.getX1()));
  ...
  public void load(Field field, WzProperty propFoothold, WzProperty ladderRope, WzProperty info) {
      ...
      for (WzProperty page : propFoothold.getChildNodes()) {
        for (WzProperty massFoothold : page.getChildNodes()) {
          for (WzProperty foothold : massFoothold.getChildNodes()) {
            ...
            if (drag > 0) pfh.getAttrFoothold().setDrag((double) drag * 0.01d);
            if (force > 0) pfh.getAttrFoothold().setForce((double) force * 0.01d);
            ...
            if (mbr.left > xMin + 30) mbr.left = xMin + 30;
            if (mbr.right < xMax - 30) mbr.right = xMax - 30;
            if (mbr.top > yMin - 300) mbr.top = yMin - 300;
            if (mbr.bottom < yMax + 10) mbr.bottom = yMax + 10;
          }
        }
      }
    }
    if (info != null) {
      ...
      if (VRLimit) {
        ...
        if (VRLeft > 0 && mbr.left < VRLeft + 20) mbr.left = VRLeft + 20;
        if (VRRight > 0 && mbr.right > VRRight) mbr.right = VRRight;
        if (VRTop > 0 && mbr.top < VRTop + 65) mbr.top = VRTop + 65;
        if (VRBottom > 0 && mbr.bottom > VRBottom) mbr.bottom = VRBottom;
      }
    }
