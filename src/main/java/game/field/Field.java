...
public class Field {
  ...
  public boolean onEnter(final User user) {
    ...
    if (!splitRegisterFieldObj(
        user.getCurrentPosition().x, user.getCurrentPosition().y, FieldSplit.User, user)) {
  ...
  public void onMobMove(User ctrl, Mob mob, InPacket packet) {
    ...
    if (mob.getController().getUser() != ctrl
        && ((mobCtrlState & 0xF0) == 0
            || mob.isNextAttackPossible()
    ...
    else action = (byte) ((action >> 1) & 0xFF);
    if (mob.onMobMove(nextAttackPossible, action, data)) {
      ctrl.sendPacket(
          MobPool.onCtrlAck(mob.getGameObjectID(), mobCtrlSN, nextAttackPossible, mob.getMP()));
      ...
      if (!mp.getElem().isEmpty()) {
        ...
        if (splitOld == null
          ...
          if (splitOld == null || centerSplit == null) {
            ...
            if (!mob.getTemplate()
                .isBoss()) { // pMob.dwTemplateID / 10000 != 880 && pMob.dwTemplateID / 10000 !=
              // 881;
        ...
        if (!mob.setMovePosition(
            tail.getX(), tail.getY(), (byte) (tail.getMoveAction() & 0xFF), tail.getFh())) {
        ...
        splitSendPacket(
            mob.getSplit(),
            MobPool.onMove(mob.getGameObjectID(), nextAttackPossible, left, data, mp),
            ctrl);
      }
      mp.getElem().clear();
    }
  ...
  public void onNpcMove(User ctrl, Npc npc, InPacket packet) {
    if (npc.getController() != null && npc.getController().getUser() == ctrl) {
      ...
      if (npc.getNpcTemplate().isMove()) {
        ...
        if (!mp.getElem().isEmpty()) {
          ...
          if (tail != null) {
            ...
            if (tail.getX() < npc.getOriginalPos().x - 50
                || tail.getX() > npc.getOriginalPos().x + 50
                || tail.getY() < npc.getOriginalPos().y - 50
                || tail.getY() > npc.getOriginalPos().y + 50) {
              Logger.logError(
                  "Invalid NPC Position [ id : %d, field : %d, pos : %d, %d ]",
                  npc.getTemplateID(), field, tail.getX(), tail.getY());
      ...
      splitSendPacket(
          ctrl.getSplit(), NpcPool.onMove(npc.getGameObjectID(), action, chatIdx, mp), ctrl);
  ...
  public void splitRegisterFieldObj(
      FieldSplit split, int foc, FieldObj objNew, OutPacket packetEnter) {
    if (split != null) {
  ...
  public void splitUnregisterFieldObj(
      FieldSplit split, int foc, FieldObj posObj, OutPacket packetLeave) {
    if (split != null) {
      for (User user : split.getUser()) {
        if (user != null) {
