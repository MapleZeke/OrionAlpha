...
public class LifePool {
  ...
  public boolean changeMobController(int characterID, Mob mobWanted, boolean chase) {
    ...
    mobWanted.sendChangeControllerPacket(
        ctrl.getUser(), chase ? MobCtrl.Active_Req : MobCtrl.Active_Int);
    return true;
  }
  ...
  public boolean createMob(Mob mob, Point pt) {
    return createMob(
        ...
        pt.x,
        pt.y,
        (short) field.getSpace2D().getFootholdUnderneath(pt.x, pt.y).getSN(),
        false,
        (byte) 0,
        0,
        null);
  ...
  public boolean createMob(
    ...
    if (template != null && owner != null) {
      ...
      if (template.getMoveAbility() == MoveAbility.Fly) {
        ...
        moveAction =
            template.getMoveAbility() == MoveAbility.Stop
                ? MoveActionType.Stand
                : MoveActionType.Move;
      }
  ...
  public void init(Field field, WzProperty mapData) {
    ...
    int mapHeight =
        Math.max(field.getMapSize().cy - Field.ScreenHeightOffset, Field.WvsScreenHeight);
    int mobCapacity =
        Math.min(
            40,
            Math.max(
                1, (int) ((double) (mapHeight * mapWidth) * field.getMobRate() * 0.0000078125d)));
    ...
    for (WzProperty life : mapData.getNode("life").getChildNodes()) {
      ...
      if (type.equalsIgnoreCase("n")) {
        ...
        if (pmg.regenInterval > 0) {
          ...
          if (6 * interval > 0) {
            pmg.regenAfter =
                System.currentTimeMillis() + interval + Rand32.getRand(6 * interval, 0);
          }
        }
        pmg.mobCount.set(0);
  ...
  public void redistributeLife() {
    ...
    while (true) {
      ...
      if (maxCtrl.getCtrlMob().isEmpty()) break;
      Mob mob = null;
  ...
  public void tryCreateMob(boolean reset) {
    ...
    if (reset || (time - lastCreateMobTime) >= 7000) {
      ...
      if (ctrlMin.getCount() > mobCapacityMin / 2) {
        mobCapacity =
            ctrlMin.getCount() < 2 * mobCapacityMin
                ? mobCapacityMin
                    + (mobCapacityMax - mobCapacityMin)
                        * (2 * ctrlMin.getCount() - mobCapacityMin)
                        / (3 * mobCapacityMin)
                : mobCapacityMax;
      } else {
  ...
  public boolean onUserAttack(
    ...
    if (user.lock()) {
      try {
        ...
        packet.encodeByte(
            SkillAccessor.getWeaponMastery(user.getCharacter(), weaponItemID, attackType, null));
        ...
        int mpSteal = 0;
        int mpStealSkill =
            SkillAccessor.getMPStealSkillData(user.getCharacter(), attackType, prop, percent);
        if (mobCount > 0) {
          for (Iterator<AttackInfo> it = attack.iterator(); it.hasNext(); ) {
            AttackInfo info = it.next();
            if (!mobs.containsKey(info.mobID)) {
            ...
            if (percent.get() != 0) {
              mpSteal +=
                  mob.onMobMPSteal(
                      prop.get(), (int) Math.ceil((double) percent.get() / (double) mobCount));
            }
            ...
            if (skill != null && slv > 0) {
              ...
              if (skillID == Thief.Steal) {
                ...
                int hp =
                    Math.min(
                        Math.min(
                            damageSum * level.getX() / 100,
                            user.getCharacter().getCharacterStat().getMHP() / 2),
                        mob.getMaxHP());
