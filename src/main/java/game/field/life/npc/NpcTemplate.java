    ...
    if (shop != null) {
      for (WzProperty items : shop.getChildNodes()) {
        ...
        if (!ItemAccessor.isRechargeableItem(itemID) && price == 0) {
          Logger.logError(
              "No Price Information in Shop List (NPC ID : %d, Item ID : %d)", templateID, itemID);
          continue;
        }
  ...
  public void encodeShop(User user, OutPacket packet) {
    ...
    for (ShopItem item : shopItem) {
      ...
      if (ItemAccessor.isRechargeableItem(item.itemID)) {
        ...
        maxPerSlot =
            SkillInfo.getInstance().getBundleItemMaxPerSlot(item.itemID, user.getCharacter());
      } else {
        ...
        switch (name) {
          case "price":
            ...
            if (!ItemAccessor.isRechargeableItem(item.itemID) && item.price == 0) {
              Logger.logError(
                  "No Price Information in Shop List (NPC ID : %d, Item ID : %d)",
                  this.templateID, item.itemID);
            }
            break;
          case "stock":
