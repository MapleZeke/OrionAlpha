...
public class MobTemplate implements WzXML {
  ...
  private static void registerMob(int templateID, WzProperty prop) {
    ...
    template.pad =
        (short) Math.min(1999, Math.max(0, WzUtil.getShort(info.getNode("PADamage"), 0)));
    template.pdd =
        (short) Math.min(1999, Math.max(0, WzUtil.getShort(info.getNode("PDDamage"), 0)));
    template.mad =
        (short) Math.min(1999, Math.max(0, WzUtil.getShort(info.getNode("MADamage"), 0)));
    template.mdd =
        (short) Math.min(1999, Math.max(0, WzUtil.getShort(info.getNode("MDDamage"), 0)));
