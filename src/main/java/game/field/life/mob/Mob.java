...
public class Mob extends Creature {
  ...
  public boolean checkIsPossibleMoveStart(User user, MovePath mp, Pointer<Boolean> result) {
    ...
    moveStart.x = x;
    moveStart.y = y;
    if (lastX == 0 || lastY == 0 || Math.abs(lastX - x) <= 500) suddenMove = false;
    lastX = tail.getX();
    lastY = tail.getY();
    if (experiencedMoveStateChange || Rand32.genRandom() % 100 >= 20) return false;
    lockMoveRect.lock();
    try {
      ...
      if (bMove || !suddenMove) bMove = true;
      result.set(bMove);
      ...
      if (getEXP() != 0) {
        ...
        for (Info info : damageLog.getLog()) {
          if (info.damage > 0) {
            ...
            if (user == null
                || user.getField() == null
                || user.getField().getFieldID() != damageLog.fieldID) {
              continue;
            }
            double lastSum = 0.0d;
            if (idx == 0) {
            ...
            if (user.lock(1600)) {
              try {
                double incEXP =
                    (((double) getEXP() * (double) info.damage) * 0.8 / (double) damageSum
                        + lastSum);
  ...
  public void giveReward(int ownerID, Point hit, short delay, boolean steal) {
    if (!alreadyStealed || !steal) {
      ...
      List<Reward> rewards =
          Reward.create(template.getRewardInfo(), false, ownerDropRate, ownerDropRate_Ticket);
      ...
      for (Reward reward : rewards) {
        getField()
            .getDropPool()
            .create(reward, ownerID, getGameObjectID(), hit.x, hit.y, x2, 0, delay, false, 0);
        x2 += 20;
      }
      if (steal) {
  ...
  public void heal(int min, int max) {
    int decHP;
    if (max == min) decHP = Rand32.genRandom().intValue();
    else decHP = min + Rand32.genRandom().intValue() % (max - min);
  ...
  public void init(MobGen mobGen, short fh) {
    ...
    if (mobGen != null && mobGen.regenInterval != 0) mobGen.mobCount.incrementAndGet();
    this.mobGen = mobGen;
  ...
  public boolean onMobHit(User user, int damage, byte attackType) {
    ...
    if (time - lastAttack > 5000
        && (controller == null || user != controller.getUser())
        && !nextAttackPossible)
    ...
    if (damage > 0) {
      if (damage > 9999) {
        Logger.logError(
            "Invalid Mob Damage. Name : %s, Damage: %d", user.getCharacterName(), damage);
        return false;
      }
  ...
  public int onMobMPSteal(int prop, int percent) {
    ...
    if (template.isBoss()) {
      ...
      if (Rand32.genRandom() % 100 >= prop || decMP < 0) decMP = 0;
      this.mp -= decMP;
    }
    return decMP;
  }
    ...
    if (time - lastAttack > 5000) {
      ...
      if (infoAdd == null || time - infoAdd.time > 5000) {
        for (Iterator<Info> it = damageLog.getLog().descendingIterator(); it.hasNext(); ) {
          Info info = it.next();
          if (time - info.time > 5000) break;
          if (info.characterID != characterID
              && getField().getLifePool().changeMobController(info.characterID, this, true))
            return false;
        }
      }
      this.lastAttack = time;
  ...
  public void onMobStatChangeSkill(User user, SkillEntry skill, byte slv, int damageSum) {
    ...
    switch (skillID) {
      case Wizard2.ColdBeam:
        {
          int attr;
          if ((attr = template.getDamagedElemAttr().get(AttackElem.Ice)) == AttackElemAttr.Damage0
              || attr == AttackElemAttr.Damage50) {
            return;
          }
          opt.setOption(1);
      ...
      case Thief.Steal:
        {
          if (skillID == Hunter.ArrowBomb
              && damageSum < template.getPushedDamage()
              && (Rand32.genRandom() % 3) != 0) {
            return;
          }
          opt.setOption(1);
      ...
      case Wizard1.PoisonBreath:
        {
          int attr;
          if ((attr = template.getDamagedElemAttr().get(AttackElem.Poison))
                  == AttackElemAttr.Damage0
              || attr == AttackElemAttr.Damage50) {
            return;
          }
          opt.setOption(
              Math.max(Math.min(Short.MAX_VALUE, getMaxHP() / (70 - slv)), level.getMAD()));
  ...
  public void sendMobTemporaryStatSet(int flag, int delay) {
    if (flag != 0) {
      getField()
          .splitSendPacket(
              getSplit(),
              MobPool.onStatSet(this, flag, stat.getStatReason(flag), (short) delay),
              null);
    }
  }
  ...
  public void update(long time) {
    if (hp <= 0) return;
    if (stat.getStatOption(MobStats.Poison) != 0) updatePoison(time);
    int reset = stat.resetTemporary(time);
    if (hp == 1) {
      if (stat.getStatOption(MobStats.Poison) != 0) reset |= stat.reset(MobStats.Poison);
    }
    if (reset != 0) {
    ...
    for (Iterator<Long> it = attackers.values().iterator(); it.hasNext(); ) {
      ...
      if (time - attackTime > 1000 * 5) it.remove();
    }
  }
