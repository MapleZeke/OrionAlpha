...
public class PortalMap {
  ...
  public boolean enablePortal(String name, boolean enable) {
    for (Portal p : portal) {
      if ((p.getPortalType() == PortalType.Changable
              || p.getPortalType() == PortalType.Changable_Invisible)
          && p.getPortalName().equalsIgnoreCase(name)) {
        p.setEnable(enable);
        return true;
      }
    }
  ...
  public Portal findCloseStartPoint(int x, int y) {
    int count = 0;
    if (!startPoint.isEmpty()) count = startPoint.size() - 1;
    int idx = -1;
    double dis = 0;
    if (count >= 0) {
      for (int i = count; i >= 0; i--) {
        ...
        double range =
            (double)
                ((x - pt.getPortalPos().x) * (x - pt.getPortalPos().x)
                    + (y - pt.getPortalPos().y) * (y - pt.getPortalPos().y));
        if (idx < 0 || range < dis) {
          dis = range;
          idx = i;
        }
  ...
  public Portal
      getRandStartPoint() { // GetRandStartPoint2 and GetRandStartPoint3 are the same and just
    // LogReport..
    ...
    for (Portal pt : portal) {
      if (pt.getPortalType() > 0) {
        if (pt.getPortalType() != PortalType.TownPortal_Point) {
          Rect rc =
              new Rect(
                  pt.getPortalPos().x - xrange,
                  pt.getPortalPos().y - 70,
                  pt.getPortalPos().x + xrange,
                  pt.getPortalPos().y + 70);
  ...
  public void resetPortal() {
    for (Portal p : portal) {
      p.setEnable(
          !(p.getPortalType() == PortalType.Changable
              || p.getPortalType() == PortalType.Changable_Invisible));
    }
  }
  ...
  public void restorePortal(WzProperty propPortal, Field field) {
    if (propPortal != null && field != null) {
      for (WzProperty prop : propPortal.getChildNodes()) {
        ...
        pt.enable =
            !(pt.getPortalType() == PortalType.Changable
                || pt.getPortalType() == PortalType.Changable_Invisible);
        pt.pos =
            new Point(WzUtil.getInt32(prop.getNode("x"), 0), WzUtil.getInt32(prop.getNode("y"), 0));
        pt.tname = WzUtil.getString(prop.getNode("tn"), "");
        pt.tmap = WzUtil.getInt32(prop.getNode("tm"), 0);
        pt.script = null;
        if (pt.getPortalType() == PortalType.Script
            || pt.getPortalType() == PortalType.Script_Invisible
            || pt.getPortalType() == PortalType.Collision_Script) {
