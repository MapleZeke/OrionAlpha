...
public class FieldMan {
  ...
  private void restoreFoothold(
      Field field, WzProperty propFoothold, WzProperty ladderOrRope, WzProperty info) {
    ...
    field.setMapSize(
        new Size(
            field.getSpace2D().getMBR().right - field.getSpace2D().getMBR().left,
            field.getSpace2D().getMBR().bottom - field.getSpace2D().getMBR().top));
  }
