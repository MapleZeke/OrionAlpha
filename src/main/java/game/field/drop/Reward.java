...
public class Reward {
  ...
  public static List<Reward> create(
      List<RewardInfo> rewardInfo,
      boolean premiumMap,
      int ownerDropRate,
      int ownerDropRate_Ticket) {
    final float INC_DROP_RATE = 1.0f; // fIncDropRate, default 1
    final float REWARD_RATE =
        1.0f; // dRewardRate, always 1 (used for monster carnival which doesn't exist)
    ...
    for (RewardInfo info : rewardInfo) {
      ...
      if (info.getType() == RewardType.MONEY) {
        ...
        if (info.getType() == RewardType.ITEM) itemRate = ownerDropRate_Ticket;
      }
      int minProb =
          (int)
              (long)
        ...
        if (!info.isPremiumMap() || premiumMap) {
          ...
          if (info.getType() == RewardType.MONEY) {
            ...
            if (reward.getType() == RewardType.ITEM) {
              ItemSlotBase item =
                  ItemInfo.getItemSlot(info.getItemId(), ItemVariationOption.Normal);
              if (item != null) {
                if (info.getPeriod() > 0) {
                ...
                if (ItemAccessor.isBundleTypeIndex(
                    ItemAccessor.getItemTypeIndexFromID(info.getItemId()))) {
                  int rand = Math.min(info.getMax() - info.getMin() + 1, 1);
                  if (rand > 1) rand = info.getMin() + (int) (Rand32.getInstance().random() % rand);
