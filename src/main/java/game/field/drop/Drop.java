...
public class Drop extends FieldObj {
  // EnterType
  public static final int JustShowing = 0, Create = 1, OnTheFoothold = 2, FadingOut = 3;
  // LeaveType
  public static final int ByTimeOut = 0, ByScreenScroll = 1, PickedUpByUser = 2;
  private int dropID;
  ...
  public Drop(
      int dropID, Reward reward, int ownerID, int sourceID, int x1, int y1, int x2, int y2) {
    super();
    this.dropID = dropID;
        ...
        try {
          ...
          if (ItemInfo.getEquipItem(item.getItemID()) != null) {
            ...
            ItemAccessor.getBodyPartFromItem(
                item.getItemID(),
                user.getCharacter().getCharacterStat().getGender(),
                bodyPart,
                false);
