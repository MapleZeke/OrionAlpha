...
public class ItemInfo {
  ...
  public static int getExclusiveClothesBodyPart(List<ItemSlotBase> a, int itemID) {
    ...
    if (itemID != 0) {
      if (ItemAccessor.isLongCoat(itemID) && excl2 != null) return BodyPart.Pants;
  ...
  public static int getExclusiveEquipItemBodyPart(List<ItemSlotBase> a, int itemID) {
    ...
    if (exclBodyPart == 0) exclBodyPart = getExclusiveClothesBodyPart(a, itemID);
    return exclBodyPart;
  }
  ...
  public static int getExclusiveWeaponShieldBodyPart(List<ItemSlotBase> a, int itemID) {
    ...
    if (itemID != 0) {
      if (itemID / 100000 == 14 && excl2 != null) return BodyPart.Shield;
  ...
  public static ItemSlotBase getItemSlot(int itemID, int option) {
    ...
    if (ti == ItemType.Equip) {
      ...
      item.ruc =
          (byte) ItemVariationOption.getVariation(info.getTUC(), option); // Total Upgrade Count
  ...
  public static boolean isAbleToEquip(
      int gender, int level, int job, int STR, int DEX, int INT, int LUK, int pop, int itemID) {
    EquipItem info = getEquipItem(itemID);
    if (info != null) {
      ...
      if (jobCategory == JobCategory.Wizard) {
        ...
        if (nWT != 0 && nWT != 30 && nWT != 33 && nWT != 31 && nWT != 32 && nWT != 38
            && nWT != 37) {
          return false;
        }
        jobBit = 2;
      ...
      return ItemAccessor.isMatchedItemIDGender(itemID, gender)
          ...
          && (info.getReqJob() == 0
              || info.getReqJob() == -1 && jobBit == 0
              || info.getReqJob() > 0 && (info.getReqJob() & jobBit) != 0);
    }
    return false;
  }
  ...
  public static boolean isReqUpgradeItem(int uItemID, int eItemID) {
    ...
    if (uItemID / 10000 != 204 || ItemAccessor.getItemTypeIndexFromID(eItemID) != ItemType.Equip)
      canUpgrade = false;
    else canUpgrade = uItemID % 10000 / 100 == eItemID / 10000 % 100;
  ...
  private static void registerEquipItemInfo(WzProperty itemData) {
    ...
    if (info != null) {
      ...
      item.setIncMaxMP(
          WzUtil.getShort(info.getNode("incMMP"), WzUtil.getShort(info.getNode("incMMD"), 0)));
      ...
      // vslot, iconRaw, tuc, sfx, incMDD, icon, reqLUK, reqLevel, knockback, reqDEX, incJump,
      // price, attack, incINT, islot, incSTR, incPDD, stand, cash, incMHP, reqPOP, afterImage,
      // incACC, incLUK, nameTag, incMMD, incDEX, reqJob, chatBalloon, incSpeed, attackSpeed, name,
      // incEVA, incMMP, incMAD, incPAD, reqINT, walk, reqSTR, desc
  ...
  private static void loadBundleItem(WzProperty itemData) {
    ...
    if (info != null) {
      ...
      // unitPrice, iconRaw, incMDD, icon, incACC, slotMax, incLUK, incDEX, incJump, price, success,
      // incSpeed, name, incINT, incSTR, incPDD, incMAD, incEVA, incPAD, cash, incMHP, desc
    }
