...
public class Inventory {
  ...
  public static boolean changeSlotPosition(
      User user, byte onExclRequest, byte ti, short pos1, short pos2, short count) {
      ...
      try {
        if (user.getHP() > 0 && pos1 != pos2) {
            ...
            if (user.getField() != null) {
              ...
              if (item != null) {
                ...
                if (InventoryManipulator.rawRemoveItem(
                    user.getCharacter(), ti, pos1, count, changeLog, decRet, itemRemoved)) {
                  if (ti == ItemType.Equip && pos1 < 0) user.validateStat(false);
                  if (itemRemoved.get() != null && user.getField() != null) {
                    ...
                    user.getField()
                        ...
                        .create(
                            ...
                            user.getCharacterID(),
                            0,
                            user.getCurrentPosition().x,
                            user.getCurrentPosition().y,
                            user.getCurrentPosition().x,
                            0,
                            0,
                            user.isGM(),
          ...
          if ((pos1 > 0 && pos1 <= b) && (pos2 > 0 && pos2 <= b)) {
            ...
            ItemSlotBase item2;
            if (item != null) {
              if ((item2 = user.getCharacter().getItem(ti, pos2)) == null
                  || ItemAccessor.isTreatSingly(item)
                  || item.getItemID() != item2.getItemID()) {
                if (user.getCharacter().getItemTrading().get(ti).get(pos1) <= 0
                    && user.getCharacter().getItemTrading().get(ti).get(pos2) <= 0) {
                  ...
                  InventoryManipulator.insertChangeLog(
                      changeLog, ChangeLog.Position, ti, pos1, null, pos2, (short) 0);
                }
              } else {
                ...
                if (ItemInfo.getBundleItem(item.getItemID()) != null) {
                  int slotMax =
                      SkillInfo.getInstance()
                              .getBundleItemMaxPerSlot(item.getItemID(), user.getCharacter())
                          - item2.getItemNumber();
                  if (slotMax <= 0) slotMax = 0;
                  int number = item.getItemNumber();
                  if (slotMax < number) number = slotMax;
                  Pointer<Integer> decRet = new Pointer<>(0);
                  if (Inventory.rawRemoveItem(
                      user, ti, pos1, (short) number, changeLog, decRet, null)) {
                    count =
                        (short)
                            (item2.getItemNumber()
                                - user.getCharacter().getItemTrading().get(ti).get(pos2));
                    if (count > 0) {
                      InventoryManipulator.insertChangeLog(
                          changeLog,
                          ChangeLog.ItemNumber,
                          ti,
                          pos2,
                          null,
                          (short) 0,
                          (short) (count + decRet.get()));
                    } else {
                      InventoryManipulator.insertChangeLog(
                          changeLog,
                          ChangeLog.NewItem,
  ...
  private static void equip(User user, short pos1, short pos2, List<ChangeLog> changeLog) {
    ...
    InventoryManipulator.insertChangeLog(
        changeLog, ChangeLog.Position, ItemType.Equip, pos2, item, pos1, (short) 0);

    if (user.getSecondaryStat().getStatOption(CharacterTemporaryStat.Booster) != 0
        && ItemAccessor.isWeapon(item.getItemID())) {
      user.sendTemporaryStatReset(
          user.getSecondaryStat().resetByCTS(CharacterTemporaryStat.Booster));
    }
  ...
  public static boolean exchange(
      User user,
      int money,
      List<ExchangeElem> exchange,
      List<ChangeLog> logAdd,
      List<ChangeLog> logRemove) {
    if (user.lock()) {
      try {
        if (user.getHP() == 0) {
          return false;
        ...
        boolean aExchangea =
            InventoryManipulator.rawExchange(
                user.getCharacter(), money, exchange, logAdd, logRemove, modFlag, logDef);
        if (aExchangea) {
          if (money != 0) {
          ...
          for (int i = 1; i <= nSlotCount; i++) {
            ...
            if (item != null && item.getItemID() == itemID) {
              if (ti != ItemType.Consume && ti != ItemType.Install && ti != ItemType.Etc) {
                ...
                count +=
                    Math.max(
                        item.getItemNumber() - user.getCharacter().getItemTrading().get(ti).get(i),
                        0);
              }
            }
          }
      ...
      try {
        ...
        if (slotCount >= 1) {
          for (int i = 1; i <= slotCount; i++) {
            if (user.getCharacter().getItemSlot(ti).get(i) == null) ++count;
          }
        }
        return count;
      } finally {
  ...
  public static boolean incItemSlotCount(User user, byte ti, int inc) {
    if (ti <= ItemType.NotDefine || ti >= ItemType.NO) return false;
    if (user.lock()) {
      try {
        int slotCount = user.getCharacter().getItemSlotCount(ti);
  ...
  public static int moveMoneyToTemp(User user, int amount) {
    if (user.lock()) {
      try {
        ...
        if (amount <= 0
            || user.getCharacter().getCharacterStat().getMoney() - moneyTrading < amount) {
          Logger.logError(
              "Invalid amount of money when move temporary space [%s] (%d)",
              user.getCharacterName(), amount);
          return 0;
        } else {
  ...
  public static boolean rawAddItem(
      User user, byte ti, ItemSlotBase item, List<ChangeLog> changeLog, Pointer<Integer> incRet) {
    if (user.getHP() == 0) {
      return false;
    }
  ...
  public static boolean rawMoveItemToTemp(
      User user,
      byte ti,
      short pos,
      short number,
      Pointer<ItemSlotBase> itemCopyed,
    ...
    if (user.lock()) {
      try {
        ...
        if (item == null
            || ti != ItemType.Consume && ti != ItemType.Install && ti != ItemType.Etc && pos < 1) {
          Logger.logError(
              "Invalid Item Position when move temporary space [%s] (%d, %d)",
              cd.getCharacterStat().getName(), ti, pos);
          return false;
        }
        if (ItemInfo.isCashItem(item.getItemID()) || item.getCashItemSN() > 0) {
          Logger.logError(
              "Invalid Item when move temporary space [%s] (%d,%d:%d)",
              cd.getCharacterStat().getName(), ti, pos, item.getItemID());
          return false;
        }
        if (ItemAccessor.isTreatSingly(item)) {
          if (number != 1 || cd.getItemTrading().get(ti).get(pos) > 0) { // only 1 here
            Logger.logError(
                "Invalid Item Count when move temporary space [%s] (%d,%d) Total:%d, Packet:%d");
            return false;
          }
          cd.getItemTrading().get(ti).set(pos, 1);
          itemCopyed.set(item.makeClone());
          InventoryManipulator.insertChangeLog(
              changeLog, ChangeLog.DelItem, ti, pos, null, (short) 0, (short) 0);
        } else {
          ...
          InventoryManipulator.insertChangeLog(
              changeLog,
              (remain - number) != 0 ? ChangeLog.ItemNumber : ChangeLog.DelItem,
              ti,
              pos,
              null,
              (short) 0,
  ...
  public static boolean rawRemoveItem(
      User user,
      byte ti,
      short pos,
      short count,
      List<ChangeLog> changeLog,
      Pointer<Integer> decRet,
      Pointer<ItemSlotBase> itemRemoved) {
    if (user.getHP() > 0)
      return InventoryManipulator.rawRemoveItem(
          user.getCharacter(), ti, pos, count, changeLog, decRet, itemRemoved);
    return false;
  }
          ...
          for (int i = 1; i <= user.getCharacter().getItemSlotCount(ti); i++) {
            ...
            if (number > 0) {
              ...
              if (item != null) {
                ...
                InventoryManipulator.insertChangeLog(
                    changeLog,
                    number != item.getItemNumber() ? ChangeLog.ItemNumber : ChangeLog.NewItem,
                    (byte) ti,
                    (short) i,
                    item,
                    (short) 0,
  ...
  public static boolean restoreItemFromTemp(
      User user, byte onExclRequest, byte ti, short pos, short number) {
    if (user.lock()) {
      try {
        List<ChangeLog> changeLog = new ArrayList<>();
            ...
            if (item != null) {
              ...
              if (ItemAccessor.isTreatSingly(item)) {
                if (number == 1) {
                  ...
                  InventoryManipulator.insertChangeLog(
                      changeLog, ChangeLog.NewItem, ti, pos, item.makeClone(), (short) 0, remain);
                  success = true;
                }
              } else {
                if (trading >= number) {
                  ...
                  InventoryManipulator.insertChangeLog(
                      changeLog,
                      item.getItemNumber() <= trading ? ChangeLog.NewItem : ChangeLog.ItemNumber,
                      ti,
                      pos,
                      item.makeClone(),
                      (short) 0,
  ...
  public static void sendInventoryOperation(
      User user, byte onExclRequest, List<ChangeLog> changeLog) {
        ...
        if (ItemInfo.isReqUpgradeItem(useItem.getItemID(), equipItem.getItemID())) {
          if (((ItemSlotEquip) equipItem).ruc > 0) {
            ...
            if (info != null) {
              Pointer<Integer> decRet = new Pointer<>(0);
              if (InventoryManipulator.rawRemoveItem(
                      cd, ItemType.Consume, upos, 1, changeLog, decRet, null)
                  && decRet.get() == 1) {
                ...
                if (Rand32.getInstance().random() % 101 <= info.getSuccess()) {
                  ...
                  item.iMaxHP =
                      (short) Math.min(Math.max(item.iMaxHP + info.getIncMaxHP(), 0), 9999);
                  ...
                  // item.iCraft = (short) Math.min(Math.max(item.iCraft + info.getIncCraft(), 0),
                  // 255);
                ...
                InventoryManipulator.insertChangeLog(
                    changeLog, ChangeLog.DelItem, ItemType.Equip, epos, null, (short) 0, (short) 0);
                InventoryManipulator.insertChangeLog(
                    changeLog, ChangeLog.NewItem, ItemType.Equip, epos, item, (short) 0, (short) 0);
                user.validateStat(false);
                success = true;
              }
            }
  ...
  public static boolean wasteItem(User user, int itemID, short count) {
    ...
    if (!ItemAccessor.isRechargeableItem(itemID)
        || (ti = ItemAccessor.getItemTypeIndexFromID(itemID)) <= ItemType.NotDefine
        || ti >= ItemType.NO) return false;
