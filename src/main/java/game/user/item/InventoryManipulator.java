...
public class InventoryManipulator {
  ...
  public static void insertChangeLog(
      List<ChangeLog> a,
      byte change,
      byte ti,
      short pos,
    ...
    if (cl.getItem() != null
        && number > 0
        && (ti == ItemType.Consume || ti == ItemType.Install || ti == ItemType.Etc))
    ...
    if (ItemAccessor.isTreatSingly(item)) {
      ...
      if (emptyPOS > 0) {
        if (ItemInfo.isCashItem(item.getItemID())) {
          if (!item.isCashItem()) Inventory.getNextSN(item, true);
        } else {
          if (item.getSN() == 0) Inventory.getNextSN(item, false);
        }
          ...
          if (inc > 0) {
            ...
            if (itemSlot.getItemNumber() - nTradingCount > 0) {
              insertChangeLog(
                  ...
                  itemSlot,
                  (short) 0,
                  (short) (inc + itemSlot.getItemNumber() - nTradingCount));
            } else {
              insertChangeLog(
                  changeLog, ChangeLog.NewItem, ti, (short) i, itemSlot, (short) 0, (short) inc);
            }
  ...
  public static boolean rawExchange(
      CharacterData cd,
      int money,
      List<ExchangeElem> exchange,
      List<ChangeLog> logAdd,
      List<ChangeLog> logRemove,
      Pointer<Integer> modFlag,
      List<ChangeLog> logDef) {
    if (cd.getCharacterStat().getHP() == 0) return false;
    if (logDef == null) logDef = new ArrayList<>();
    if (logAdd == null) logAdd = logDef;
    if (logRemove == null) logRemove = logDef;
    if (modFlag == null) modFlag = new Pointer<>();
    ...
    for (ExchangeElem e : exchange) {
      if (!e.add) {
        ...
        if (itemID == 0) {
          ...
          if (ti <= ItemType.NotDefine
              || ti > ItemType.NO
              || !rawRemoveItem(cd, ti, e.r.pos, count, logRemove, decRet, null)
              || decRet.get() != count) break;
        ...
        ItemSlotBase item;
        if (itemID != 0 && e.a.count <= 0 || ItemAccessor.isRechargeableItem(itemID)) break;
        if (itemID == 0) item = e.a.item;
        else item = ItemInfo.getItemSlot(itemID, ItemVariationOption.None);
        if (item == null) break;
        if (e.a.itemID != 0) {
        ...
        short count;
        if (ItemAccessor.isTreatSingly(item)) count = 1;
        else count = item.getItemNumber();
  ...
  public static boolean rawIncMoney(CharacterData cd, int inc, boolean onlyFull) {
    ...
    if (inc <= 0) {
      ...
      if (money < cd.getMoneyTrading()) money = cd.getMoneyTrading();
    }
  ...
  public static boolean rawRechargeItem(CharacterData cd, short pos, List<ChangeLog> changeLog) {
    ...
    short maxPerSlot;
    if (item == null
        || ItemInfo.isCashItem(item.getItemID())
        || !ItemAccessor.isRechargeableItem(item.getItemID())
        || item.getItemNumber()
            > (maxPerSlot =
                (short) SkillInfo.getInstance().getBundleItemMaxPerSlot(item.getItemID(), cd))) {
      return false;
    } else {
  ...
  public static boolean rawRemoveItem(
    ...
    if (ItemAccessor.isTreatSingly(item)) {
      if (count == 1) {
        ...
        if (itemRemoved != null) itemRemoved.set(item.makeClone());
        insertChangeLog(changeLog, ChangeLog.DelItem, ti, pos, item, (short) 0, (short) 0);
        if (decRet != null) decRet.set(1);
        return true;
      } else {
        return false;
      }
    ...
    if (dec > 0) {
      ...
      if (decRet != null) decRet.set(dec);
      return true;
    }
    return false;
  }

  public static boolean rawWasteItem(
      CharacterData cd, short pos, short count, List<ChangeLog> aChangeLog) {
    if (count <= 0) return false;
    ItemSlotBase item = cd.getItem(ItemType.Consume, pos);
    if (item == null
        || ItemInfo.isCashItem(item.getItemID())
        || item.isCashItem()
        || !ItemAccessor.isRechargeableItem(item.getItemID())
        || cd.getItemTrading().get(ItemType.Consume).get(pos) > 0) return false;
    ...
    item.setItemNumber(remain);
    insertChangeLog(
        aChangeLog, ChangeLog.ItemNumber, ItemType.Consume, pos, null, (short) 0, remain);
    return true;
  }
}
