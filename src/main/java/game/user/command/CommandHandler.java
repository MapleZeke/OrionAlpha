...
public class CommandHandler {
  ...
  public static void handle(User user, String input) {
    ...
    String command =
        split[0].substring(1); // input is expected to have some type of command prefix.
        ...
        for (Method method : clazz.getMethods()) {
          ...
          if (usedAlias != null || method.getName().equalsIgnoreCase(command)) {
            ...
            for (Parameter param : method.getParameters()) {
              ...
              if (param
                  .getType()
                  .isAssignableFrom(User.class)) { // Automatically fill in User parameter
                methodArgs[index] = user;
                continue;
              }
              if (param
                  .getType()
                  .isAssignableFrom(String[].class)) { // Automatically fill in String[] parameter.
  ...
  private static Object findFieldFor(Object parent, List<Field> checked, Class<?> target)
      throws Exception {
    ...
    for (Field f : fields) {
      ...
      if (f.getType()
          .isAssignableFrom(target)) { // If the field is what we want grab it and return.
        fields.clear();
        return f.get(parent);
      }
      Object val = f.get(parent);
      if (val
          != null) { // Do some recursive of values inside fields so we can go from User > DropPool
