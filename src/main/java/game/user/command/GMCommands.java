...
public class GMCommands {
  ...
  public static String item(
      String alias, User user, CharacterData cd, DropPool pool, String[] args) {
    if (alias.equals("drop") || alias.equals("item")) {
      if (args.length > 0) {
        ...
        if (itemName == null || itemName.isEmpty()) itemName = String.valueOf(itemID);
        if (item != null) {
            ...
            if (args.length > 1) {
              quantity =
                  Math.max(
                      1,
                      Math.min(
                          Integer.parseInt(args[1]), ItemInfo.getBundleItem(itemID).getSlotMax()));
            }
