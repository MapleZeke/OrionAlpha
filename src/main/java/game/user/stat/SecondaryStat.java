...
public class SecondaryStat {
  ...
  public static final int MovementAffecting, FilterForRemote;
  ...
  public int resetByReasonID(int reasonID) {
    int reset = 0;
    for (Iterator<Map.Entry<Integer, SecondaryStatOption>> it = stats.entrySet().iterator();
        it.hasNext(); ) {
  ...
  public void setFrom(BasicStat bs, List<ItemSlotBase> realEquip, CharacterData cd) {
    ...
    slv =
        SkillInfo.getInstance()
            .getSkillLevel(cd, Archer.AmazonBlessing, new Pointer<>(archerAmazonBlessing));
    if (slv != 0) {
    ...
    slv =
        SkillInfo.getInstance().getSkillLevel(cd, Rogue.NimbleBody, new Pointer<>(thiefNimbleBody));
    if (slv != 0) {
    ...
    if (jc == JobCategory.Archer || job / 10 == 41) attackType = 2; // SHOOT
    int weaponItemID = 0;
    ItemSlotBase item = realEquip.get(BodyPart.Weapon);
    if (item != null) weaponItemID = item.getItemID();
