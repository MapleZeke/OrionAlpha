...
public class CalcDamage {
  ...
  public boolean checkMDamageMiss(
      MobStat ms, CharacterData cd, BasicStat bs, SecondaryStat ss, long randForMissCheck) {
    int eva =
        Math.max(
            0,
            Math.min(ss.eva + ss.getStatOption(CharacterTemporaryStat.EVA), SkillAccessor.EVA_MAX));
    int level = bs.getLevel();
    eva -= (ms.getLevel() - level) / 2;
    int mobACC = 0;
    if (level >= ms.getLevel() || eva > 0) mobACC = eva;
    double rand = (double) mobACC;
    double b = rand * 0.1d;
    rand = getRand(randForMissCheck, b, rand);
    int acc =
        Math.max(0, Math.min(ms.getACC() + ms.getStatOption(MobStats.ACC), SkillAccessor.ACC_MAX));
    return rand >= (double) acc;
  }

  public boolean checkPDamageMiss(
      MobStat ms, CharacterData cd, BasicStat bs, SecondaryStat ss, long randForMissCheck) {
    int eva =
        Math.max(
            0,
  ...
  public void PDamage(
    ...
    int acc =
        Math.max(
            0,
            Math.min(ss.acc + ss.getStatOption(CharacterTemporaryStat.ACC), SkillAccessor.ACC_MAX));
    int mobPDD = acc;
    ...
    if (bulletItemID != 0) pad += ItemInfo.getBulletPAD(bulletItemID);
    ...
    for (short dmg : damage) {
      mobPDD =
          Math.max(
              0, Math.min(ms.getEVA() + ms.getStatOption(MobStats.EVA), SkillAccessor.EVA_MAX));
