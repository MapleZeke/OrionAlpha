...
public class User extends Creature {
   ...
   *
   * <p>WARNING: This method should ONLY be used when you NULL the User object.
   */
  public final void destructUser() {
    ...
    for (List<Integer> itemTrading : character.getItemTrading()) itemTrading.clear();
    character.getItemTrading().clear();
    // character.getQuestRecord().clear();
    character.getSkillRecord().clear();
    for (List<ItemSlotBase> itemSlot : character.getItemSlot()) itemSlot.clear();
  ...
  public int incEXP(int inc, boolean onlyFull) {
    if (lock(1500)) {
      try {
        ...
        if (getHP() > 0
            && inc > 0
            && (!onlyFull || inc + character.getCharacterStat().getEXP() >= 0)) {
          if (inc < 0) {
          ...
          if (ExpAccessor.tryProcessLevelUp(character, basicStat, inc, reachMaxLev)) {
            ...
            flag |=
                CharacterStatType.HP
                    | CharacterStatType.MP
                    | CharacterStatType.MHP
                    | CharacterStatType.MMP;
            validateStat(false);
            onLevelUp();
            if (reachMaxLev.get()) {
              setMaxLevelReach();
  ...
  public int initEXP() {
    if (lock()) {
      try {
        if (character.getCharacterStat().getLevel() >= ExpAccessor.MAX_LEVEL
            && character.getCharacterStat().getEXP() > 0) {
          character.getCharacterStat().setEXP(0);
          characterDataModFlag |= DBChar.Character;
  ...
  public void statChange(int inc, int dec, short incHP, short incMP) {
    ...
    try {
      ...
      if (inc == CharacterStatType.MHP || dec == CharacterStatType.MHP)
        character
            .getCharacterStat()
            .setMHP((short) (character.getCharacterStat().getMHP() + incHP));
      if (inc == CharacterStatType.MMP || dec == CharacterStatType.MMP)
        character
            .getCharacterStat()
            .setMMP((short) (character.getCharacterStat().getMMP() + incMP));
    } finally {
      unlock();
    }
  }
      ...
      if (getField().getForcedReturnFieldID() == Field.Invalid) {
        ...
        if (lock()) {
          try {
            ...
            Field field =
                FieldMan.getInstance(getChannelID())
                    .getField(getField().getForcedReturnFieldID(), false);
            byte portal = 0;
  ...
  public void onMigrateInSuccess() {
    ...
    if (portalIdx <= 0) portalIdx = 0;
    int idx = count - 1;
    if (portalIdx < idx) idx = portalIdx;
            ...
            { // Create Item
              ...
              if (item != null) {
                if (item.getType() == ItemSlotType.Bundle) {
                  item.setItemNumber(
                      SkillInfo.getInstance().getBundleItemMaxPerSlot(itemID, character));
                }
  ...
  public void onAttack(byte type, InPacket packet) {
    ...
    if (getHP() > 0 && getField() != null) {
      if (skillID == Fighter.FinalAttack
          || skillID == Fighter.FinalAttackEx
          || skillID == Page.FinalAttack
          || skillID == Page.FinalAttackEx
          || skillID == Spearman.FinalAttack
          || skillID == Spearman.FinalAttackEx
          || skillID == Hunter.FinalAttack_Bow
          || skillID == Crossbowman.FinalAttack_Crossbow)
      ...
      if (attackCheckIgnoreCnt <= 0) {
        if (attackTime - lastAttackTime >= lastAttackDelay) {
          ...
          if (attackSpeedErr == 2) {
            Logger.logError(
                "[ User ] user's attack speed is abnormally fast [ name=%s, actionNo=%d, skillID=%d, userDelay=%d < minDelay=%d, finalAttack=%d, boosterLevel=%d, fieldid=%d ]");
            return;
          }
          attackSpeedErr++;
        }
          ...
          try {
            ...
            if (slv > 0) {
              ...
              if (skillID != Cleric.Heal
                  && !SkillInfo.getInstance().adjustConsumeForActiveSkill(this, skillID, slv)) {
                Logger.logReport(
                    "Failed to adjust consume for active skill!!! (SkillID:%d,SLV:%d)",
                    skillID, slv);
                getCalcDamage().skip();
                return;
              }
      ...
      if (maxCount < mobCount) {
        ...
        if (bulletItemPos > 0) {
          ...
          if (item != null) {
            if (character.getItemTrading().get(ItemType.Consume).get(bulletItemPos) != 0
                || item.getItemNumber() < bulletCount) {
              getCalcDamage().skip();
              return;
            }
            if (ItemAccessor.isJavelinItem(item.getItemID())) {
              ...
              if (Inventory.rawRemoveItem(
                      ...
                      decRet,
                      null)
                  && decRet.get() == bulletCount) {
                addCharacterDataMod(DBChar.ItemSlotConsume);
                bulletItemID = item.getItemID();
                Inventory.sendInventoryOperation(this, Request.None, changeLog);
              } else {
                Logger.logError(
                    "Invalid skill info in attack packet (nItemPos: %d, nCount: %d, nDecRet: %d)",
                    bulletItemPos, bulletCount, decRet.get());
              }
            }
            changeLog.clear();
          }
    ...
    if (lock()) {
      try {
        ...
        if (hp > 0) {
          ...
          if (JobAccessor.getJobCategory(character.getCharacterStat().getJob())
                  == JobCategory.Fighter
          ...
          int recoveryHP =
              (int)
                  ((double) (SkillAccessor.getHPRecoveryUpgrade(character) + 10.0d) * recoveryRate);
          if (recoveryHP < hp) {
            ++illegalHPIncSize;
          }
        ...
        if (mp > 0) {
          ...
          int recoveryMP =
              (int)
                  ((double) (SkillAccessor.getMPRecoveryUpgrade(character) + 3.0d) * recoveryRate);
          if (recoveryMP < mp) {
            ++illegalMPIncSize;
          }
  ...
  public void onConsumeCashItemUseRequest(InPacket packet) {
    ...
    List<ChangeLog> changeLog = new ArrayList<>();
    Pointer<Integer> decRet = new Pointer<>(0);
    if (Inventory.rawRemoveItem(this, ItemType.Consume, pos, (short) 1, changeLog, decRet, null)
        && decRet.get() == 1) {
  ...
  public void onEmotion(InPacket packet) {
    if (getField() != null) {
      ...
      if (emotion < 8) {
        getField()
            .splitSendPacket(getSplit(), UserRemote.onEmotion(this.characterID, emotion), this);
      }
    }
  }
  ...
  public void onDropMoneyRequest(InPacket packet) {
    ...
    if (amount >= 10 && amount <= 50000) {
      ...
      if (getField() != null) {
        ...
        if (getField()
                .getSpace2D()
                .getFootholdUnderneath(getCurrentPosition().x, getCurrentPosition().y, y2)
            != null) {
          ...
          getField()
              .getDropPool()
              .create(reward, this.characterID, 0, x, y1, x, y2.get(), 0, false, 0);
        }
      }
    }
  }
  ...
  public void onGivePopularityRequest(InPacket packet) {
    ...
    if (target == null || target == this) {
      sendPacket(
          WvsContext.onGivePopularityResult(GivePopularityRes.InvalidCharacterID, null, false));
    } else {
      if (character.getCharacterStat().getLevel() < 15) {
        ...
        if (ret == GivePopularityRes.Success) {
          ...
          target.sendPacket(
              WvsContext.onGivePopularityResult(
                  GivePopularityRes.Notify, getCharacterName(), incFame));

          sendPacket(
              WvsContext.onGivePopularityResult(
                  GivePopularityRes.Success, target.getCharacterName(), incFame));
        } else {
        ...
        if (getHP() > 0) {
          if (clientDamage > 0) {
            ...
            if (template != null) {
              ...
              if (mobAttackIdx >= AttackIndex.Mob_Physical
                  && !template.getAttackInfo().isEmpty()
                  && mobAttackIdx < template.getAttackInfo().size()) {
  ...
  public void onMigrateToCashShopRequest() {
    if (getField() == null
        || getSocket() == null
        || this.nexonClubID == null
        || this.nexonClubID.isEmpty()) {
      return;
    }
    ...
    if (lock()) {
      try {
        ...
        if (item == null
            ...
            || info == null
            || info.getItemID() != itemID) {
          Logger.logError(
              "Incorrect portal-scroll-item use request nPOS(%d), nItemID(%d), pInfo(%p)",
              pos, itemID);
        ...
        if (!canAttachAdditionalProcess()
          ...
          if (FieldMan.getInstance(getChannelID()).isConnected(getField().getFieldID(), fieldID)) {
            ...
            Pointer<Integer> decRet = new Pointer<>(0);
            if (Inventory.rawRemoveItem(
                    this, ItemType.Consume, pos, (short) 1, changeLog, decRet, null)
                && decRet.get() == 1) {
              Inventory.sendInventoryOperation(this, Request.None, changeLog);
              addCharacterDataMod(DBChar.ItemSlotConsume);
  ...
  public void onStatChangeItemUseRequest(InPacket packet) {
    if (getHP() == 0
        || getField() == null
        || secondaryStat.getStatOption(CharacterTemporaryStat.DarkSight) != 0) {
      sendCharacterStat(Request.Excl, 0);
      return;
    }
    short pos = packet.decodeShort();
    ...
    List<ChangeLog> changeLog = new ArrayList<>();
    Pointer<Integer> decRet = new Pointer<>(0);
    if (!InventoryManipulator.rawRemoveItem(
            character, ItemType.Consume, pos, 1, changeLog, decRet, new Pointer<>())
        || decRet.get() != 1) {
      changeLog.clear();
      sendCharacterStat(Request.Excl, 0);
      return;
  ...
  public void onSelectNpc(InPacket packet) {
    if (getField() != null) {
      ...
      if (lastSelectNPCTime == 0 || time <= lastSelectNPCTime || lastSelectNPCTime <= time - 500) {
        ...
        if (npc != null) {
          if (npc.getNpcTemplate().getQuest() != null
              && !npc.getNpcTemplate().getQuest().isEmpty()) {
          ...
          case ShopResCode.Buy:
            {
              ...
              if (pos < 0
                  || npcTemplate.getShopItem().isEmpty()
                  || pos >= npcTemplate.getShopItem().size()
                  || (shopItem = npcTemplate.getShopItem().get(pos)).itemID != itemID
                  || count <= 0
                  || (!ItemAccessor.isBundleTypeIndex(ti)
                          || ItemAccessor.isRechargeableItem(itemID))
                      && count != 1) {
              ...
              if (stockPrice <= 0
                  || character.getCharacterStat().getMoney() - character.getMoneyTrading()
                      < stockPrice) {
                ...
                if (npc.decShopItemCount(shopItem.itemID, count)) {
                  ...
                  ItemSlotBase item =
                      ItemInfo.getItemSlot(shopItem.itemID, ItemVariationOption.None);
                  if (item == null) {
                  ...
                  if (ItemAccessor.isRechargeableItem(shopItem.itemID)) {
                    int number =
                        SkillInfo.getInstance().getBundleItemMaxPerSlot(shopItem.itemID, character);
          ...
          case ShopResCode.Sell:
            {
              ...
              if (item == null || item.getItemID() != itemID) {
                ...
                Logger.logError(
                    "Selling Invalid Item in Shop [%s] (%d,%d:%d)",
                    characterName, pos, itemID, count);
                closeSocket();
                return;
              }
              int inc;
              ...
              if (money > 0 && uInc == 0 && inc >= 0) {
                ...
                if (!InventoryManipulator.rawRemoveItem(
                        character, ti, pos, count, changeLog, decRet, null)
                    || decRet.get() < count) {
          ...
          case ShopResCode.Recharge:
            {
              ...
              int maxPerSlot =
                  SkillInfo.getInstance().getBundleItemMaxPerSlot(item.getItemID(), character);
  ...
  public void onWhisper(InPacket packet) {
    ...
    if (flag == WhisperFlags.ReplyRequest) {
      ...
      if (user != null && user.getField() != null && user.getField().getFieldID() != 0) {
        user.sendPacket(
            FieldPacket.onWhisper(
                WhisperFlags.ReplyReceive, null, getCharacterName(), text, -1, false));
      ...
      if (fieldID > 0) {
        sendPacket(
            FieldPacket.onWhisper(WhisperFlags.FindResult, target, null, null, fieldID, false));
      }
    ...
    if (lock()) {
      try {
        if (field != null) {
          ...
          onTransferField(
              field, portal.getPortalPos().x, portal.getPortalPos().y, portal.getPortalIdx());
        }
      } finally {
        unlock();
      }
  ...
  public void postTransferField(
      Integer fieldID, String portal, boolean force, Point pos, boolean loopback, InPacket packet) {
    if (lock()) {
      try {
        if (!onTransferField || force) {
          if (fieldID != null && FieldMan.getInstance(getChannelID()).isBlockedMap(fieldID)) {
            Logger.logError(
                "User tried to enter the Blocked Map #3 (From:%d,To:%d)",
                getField().getFieldID(), fieldID);
            sendPacket(FieldPacket.onTransferFieldReqIgnored());
          } else {
            if (getField() == null || socket == null) {
              closeSocket();
            ...
            if (field == null) {
              Logger.logError(
                  "Invalid Field ID *1* : %d (Old: %d, IsDead: %b)",
                  fieldID, getField().getFieldID(), isDead);
              closeSocket();
              return;
            }
            if (FieldMan.getInstance(getChannelID()).isBlockedMap(fieldID)) {
              Logger.logError(
                  "User tried to enter the Blocked Map #1 (From:%d,To:%d)",
                  getField().getFieldID(), fieldID);
              sendPacket(FieldPacket.onTransferFieldReqIgnored());
              this.onTransferField = false;
              return;
            ...
            if (portal == null
                || portal.isEmpty()
                || (pt = field.getPortal().findPortal(portal)) == null) {
              if (pos.x == 0 && pos.y == 0) {
  ...
  public void sendCharacterStat(byte request, int flag) {
    ...
    try {
      character
          .getCharacterStat()
          .setMoney(character.getCharacterStat().getMoney() - character.getMoneyTrading());
  ...
  public boolean sendDropPickUpResultPacket(Drop pr, byte onExclRequest) {
    ...
    try {
      ...
      if (pr.isMoney()) {
        ...
        if (pr.getMoney() == 0) {
          if (pr.getItem() != null) money = pr.getItem().getItemID(); // wtf u doing here nexon
          else money = 0;
        }
      ...
      if (pr.isMoney() || incRet.get() > 0) {
        sendPacket(
            WvsContext.onDropPickUpMessage(
                pr.isMoney() ? DropPickup.Messo : DropPickup.AddInventoryItem,
                pr.getDropInfo(),
                pr.getItem() != null ? pr.getItem().getItemID() : 0,
                incRet.get()));
      } else {
  ...
  public boolean isWearItemOnNeed(int necessaryItemID) {
    int i = 0;
    while (character.getEquipped().get(i) == null
        || character.getEquipped().get(i).getItemID() != necessaryItemID) {
      ++i;
      if (i > BodyPart.BP_Count) return false;
    }
    return true;
  }
  ...
  public boolean isItemExist(byte ti, int itemID) {
    ...
    if (!exist) {
      if (ti == ItemType.Equip) {
        ...
        ItemAccessor.getBodyPartFromItem(
            itemID, character.getCharacterStat().getGender(), bodyPart, false);
        ItemSlotBase item = character.getItem(ti, -bodyPart.get());
        if (item != null) {
          if (item.getItemID() == itemID) exist = true;
        }
      }
    }
    return exist;
  ...
  private void onUserDead() {
    ...
    ExpAccessor.decreaseExp(
        character,
        basicStat,
        getField().getLifePool().getMobGenCount() <= 0 || getField().isTown());
  ...
  public byte tryChangeHairOrFace(int couponItemID, int param) {
    ...
    for (int pos = 1; pos <= slotCount; pos++) {
      ...
      if (item != null
          && item.getItemID() == couponItemID
          && item.isCashItem()
          && character.getItemTrading().get(ItemType.Etc).get(pos) == 0) {
        Pointer<Integer> decRet = new Pointer<>(0);
        List<ChangeLog> changeLog = new ArrayList<>();
        if (Inventory.rawRemoveItem(
                this, ItemType.Etc, (short) pos, (short) 1, changeLog, decRet, null)
            && decRet.get() == 1) {
        ...
        if (force || time - lastCharacterDataFlush >= 300000) {
          ...
          if (characterDataModFlag != 0) {
            ...
            if ((characterDataModFlag & DBChar.ItemSlotEquip) != 0) {
              CommonDB.rawUpdateItemEquip(
                  characterID,
                  character.getEquipped(),
                  character.getEquipped2(),
                  character.getItemSlot().get(ItemType.Equip));
            }
            if ((characterDataModFlag & DBChar.ItemSlotConsume) != 0
                || (characterDataModFlag & DBChar.ItemSlotInstall) != 0
  ...
  public void resetTemporaryStat(long time, int reasonID) {
    ...
    try {
      int reset;
      if (reasonID > 0) reset = secondaryStat.resetByReasonID(reasonID);
      else reset = secondaryStat.resetByTime(time);
      if (reset != 0) {
        validateStat(false);
        sendTemporaryStatReset(reset);
      }
    ...
    if (set != 0) {
      ...
      try {
        ...
        if (getField() != null) {
          getField()
              .splitSendPacket(
                  getSplit(), UserRemote.onTemporaryStatSet(characterID, secondaryStat, set), this);
        }
      } finally {
        unlock();
      }
    ...
    if (reset != 0) {
      ...
      try {
        ...
        if (getField() != null) {
          getField()
              .splitSendPacket(
                  getSplit(), UserRemote.onResetTemporaryStat(characterID, reset), this);
        }
      } finally {
        unlock();
      }
  ...
  public void onUserEffect(boolean local, boolean remote, byte effect, int... args) {
    ...
    if (remote) {
      getField()
          .splitSendPacket(
              getSplit(), UserRemote.onEffect(getCharacterID(), effect, skillID, slv), this);
    }
    if (local) {
  ...
  public void postAvatarModified(int flag) {
    if (((flag | avatarModFlag) != avatarModFlag) || flag == AvatarLook.Look) {
      avatarModFlag |= flag;
      avatarLook.load(
          character.getCharacterStat(), character.getEquipped(), character.getEquipped2());
      if (msMessenger) userMSM.notifyAvatarChanged();
  ...
  public final void validateStat(boolean calledByConstructor) {
    ...
    try {
      ...
      if (!calledByConstructor) {
        if (flag != 0) sendCharacterStat(Request.None, flag);
      ...
      if (isGM() && character.getSkillRecord().isEmpty()) {
        for (SkillEntry skill : SkillInfo.getInstance().getAllSkills()) {
          ...
          if (JobAccessor.isCorrectJobForSkillRoot(
              character.getCharacterStat().getJob(), skillRoot)) {
