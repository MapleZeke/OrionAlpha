...
public class SkillInfo {
  ...
  public boolean adjustConsumeForActiveSkill(User user, int skillID, byte slv) {
    ...
    if (user.lock()) {
      try {
        ...
        if (user.getHP() == 0
            || hp > 0 && hp >= user.getHP()
            || mp > 0 && mp > user.getCharacter().getCharacterStat().getMP()) {
          if (flag != 0) user.sendCharacterStat(Request.None, flag);
          return false;
        }
        if (hp > 0) {
          user.incHP(-hp, true);
  ...
  public int getSkillLevel(CharacterData cd, int skillID, Pointer<SkillEntry> skillEntry) {
    ...
    if (skillEntry != null) skill = skillEntry.get();
    if (skill == null) skill = skills.get(skillID);
    if (skillEntry != null && skillEntry.get() == null) skillEntry.set(skill);
