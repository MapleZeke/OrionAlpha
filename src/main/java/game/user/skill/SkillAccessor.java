...
public class SkillAccessor {
   ...
   *
   * <p>The formulation of the array is as follows: [Job Category][Level Up][Inc Val]
   *
   * <p>Where IncVal arrays are as follows: [Min HP Inc, MaxHP Inc, HP Inc Modulo Rand] [Min MP Inc,
   * Max MP Inc, MP Inc Modulo Rand]
   *
   * <p>The numbers will always follow the same sequence for each job.
   */
  ...
  public static int getEndureDuration(CharacterData cd) {
    ...
    if (jobCategory == JobCategory.Fighter) {
      ...
      if (JobAccessor.isCorrectJobForSkillRoot(
          cd.getCharacterStat().getJob(), JobAccessor.Assassin.getJob())) {
        int slv = SkillInfo.getInstance().getSkillLevel(cd, Assassin.Endure, skillEntry);
        if (slv > 0) {
          return 1000 * skillEntry.get().getLevelData(slv).getTime();
        }
      } else if (JobAccessor.isCorrectJobForSkillRoot(
          cd.getCharacterStat().getJob(), JobAccessor.Thief.getJob())) {
  ...
  public static int getMPRecoveryUpgrade(CharacterData cd) {
    ...
    if (jobCategory == JobCategory.Wizard) {
      return (int)
          ((double) SkillInfo.getInstance().getSkillLevel(cd, Magician.ImproveBasic, null)
              * (double) cd.getCharacterStat().getLevel()
              * 0.1d);
    } else if (jobCategory == JobCategory.Thief) {
      if (JobAccessor.isCorrectJobForSkillRoot(
          cd.getCharacterStat().getJob(), JobAccessor.Assassin.getJob())) {
  ...
  public static boolean incMaxHPMP(CharacterData cd, BasicStat bs, int flag, boolean levelUp) {
    ...
    if (JobAccessor.findJob(job) != null) {
      ...
      int randHP =
          INC_HP_MP[jobCategory][!levelUp ? 1 : 0][
              2]; // Useless, always 0 and Nexon only has a MP rand for INT.
      ...
      if (incMP) {
        mpInc =
            ThreadLocalRandom.current().nextInt(Short.MAX_VALUE) % (maxMP - minMP + 1)
                + minMP
                + bs.getINT() * randMP / 200;
      }
      if (incHP) {
        ...
        if (cd.getSkillRecord().containsKey(skillID)) {
          ...
          if ((skillLevel = cd.getSkillRecord().get(skillID)) > 0 && hpIncSkill != null) {
            ...
            if (levelUp) hpInc += hpIncSkill.getLevelData(skillLevel).getX();
            else hpInc += hpIncSkill.getLevelData(skillLevel).getY();
          }
        }
      }
      if (incMP) {
        ...
        if (cd.getSkillRecord().containsKey(skillID)) {
          ...
          if ((skillLevel = cd.getSkillRecord().get(skillID)) > 0 && mpIncSkill != null) {
            ...
            if (levelUp) mpInc += mpIncSkill.getLevelData(skillLevel).getX();
            else mpInc += mpIncSkill.getLevelData(skillLevel).getY();
          }
        }
      }
      if (incHP) {
        if (cd.getCharacterStat().getMHP() < HP_MAX) {
          cd.getCharacterStat()
              .setMHP(
                  (short) Math.min(Math.max(hpInc + cd.getCharacterStat().getMHP(), 50), HP_MAX));
          inc = true;
        }
      }
      if (incMP) {
        if (cd.getCharacterStat().getMMP() < MP_MAX) {
          cd.getCharacterStat()
              .setMMP(
                  (short) Math.min(Math.max(mpInc + cd.getCharacterStat().getMMP(), 5), MP_MAX));
          inc = true;
        }
      }
    }
  ...
  public static int getMPStealSkillData(
      CharacterData cd, int attackType, Pointer<Integer> prop, Pointer<Integer> percent) {
    prop.set(0);
    percent.set(0);
    if (attackType != 3) return 0;
  ...
  public static int getWeaponMastery(
      CharacterData cd, int weaponItemID, int attackType, Pointer<Integer> accInc) {
