...
public class TradingRoom extends MiniRoomBase {
  ...
  public int doTrade() {
    ...
    if (owner == null || user == null) {
      Logger.logError(
          "Cannot get a user information on trade with %s",
          owner != null
              ? owner.getCharacterName()
              : user != null ? user.getCharacterName() : "null");
    ...
    for (int i = 0; i < 2; i++) {
      for (int j = 0; j < 9; j++) {
        ...
        ItemSlotBase itemBase =
            getUsers().get(i).getCharacter().getItemSlot(tradeItem.getTI()).get(tradeItem.getPOS());
        if (itemBase == null) {
      ...
      int idx = (i > 0) ? 0 : 1;
      long meso =
          getUsers().get(i).getCharacter().getCharacterStat().getMoney()
              - getUsers().get(i).getCharacter().getMoneyTrading()
              + (mesoTrading[idx] - mesoTrading[i]);
    ...
    if (result == MiniRoomLeave.TradeDone) {
      if (!Inventory.exchange(owner, 0, exchange, logAdd, null)
          || !Inventory.exchange(user, 0, exchange2, changeLog, null)) {
        result = MiniRoomLeave.TradeFail;
      }
    }
    if (result != MiniRoomLeave.TradeDone) {
      ...
      for (int i = 0; i < 2; i++) {
        ...
        // addTotalTax
        Inventory.sendInventoryOperation(
            getUsers().get(i), Request.None, i == 0 ? logAdd : changeLog);
  ...
  private void onPutItem(User user, InPacket packet) {
    ...
    int idx;
    if (getCurUsers() == 0
        || lock[(idx = findUserSlot(user))]
        || getUsers().get(idx > 0 ? 0 : 1) == null) {
      user.sendCharacterStat(Request.Excl, 0);
      return;
    }
    ItemSlotBase itemBase = user.getCharacter().getItem(ti, slot);
    if (itemBase == null
        || (number > itemBase.getItemNumber() || number < 0)
        || (number == 0 && !ItemAccessor.isRechargeableItem(itemBase.getItemID()))
        || itemBase.getCashItemSN() > 0) {
      user.sendCharacterStat(Request.Excl, 0);
      return;
    }
  ...
  private void onPutMoney(User user, InPacket packet) {
    int idx;
    if (getCurUsers() > 0
        && (!lock[idx = findUserSlot(user)] && getUsers().get(idx > 0 ? 0 : 1) != null)) {
