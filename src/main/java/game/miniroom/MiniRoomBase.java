...
public abstract class MiniRoomBase {
  ...
  public static int create(
      User userZero, byte type, InPacket packet, boolean tournament, int round) {
    ...
    if (result == 0) {
      ...
      userOne.sendPacket(
          MiniRoomBaseDlg.onInviteStatic(userZero.getCharacterName(), miniroom.miniRoomSN));
    } else if (result == 1) {
      userZero.sendPacket(
          MiniRoomBaseDlg.onInviteResultStatic(
              MiniRoomInvite.CannotInvite, userOne.getCharacterName()));
    }
    return 0;
  }
  ...
  private int findEmptySlot(int characterID) {
    ...
    try {
      ...
      if (maxUsers <= 1) {
        ...
        while (users.get(idx) != null
            || reserved.get(idx) > 0 && characterID > 0 && reserved.get(idx) != characterID) {
          ++idx;
          if (idx >= maxUsers) {
            idx = -1;
            break;
