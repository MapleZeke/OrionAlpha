...
public class ScriptSysFunc {
  ...
  private void makeMessagePacket(int type, List<Object> memory, GameObject speaker) {
    ...
    if (!runningVM.getMsgHistory().isEmpty()
        && (type != ScriptMessage.Say
            || runningVM.getMsgHistory().getFirst().getType() != ScriptMessage.Say)) {
      clearMsgHistory();
    }
    ...
    for (int i = 0; i < argCount; i++) {
      ...
      if (count <= 0) {
        ...
        if (!ItemAccessor.isBundleTypeIndex(ti)) {
          ...
          int maxPerSlot =
              SkillInfo.getInstance().getBundleItemMaxPerSlot(itemID, getUser().getCharacter());
          if (maxPerSlot <= 0) {
            logRemove.clear();
            logAdd.clear();
            items.clear();
          ...
          if (ItemAccessor.isRechargeableItem(itemID)) {
            ...
            while (true) {
              ...
              if (j > roomCount) {
                if (count > maxPerSlot) {
                  Logger.logError(
                      "Invalid argument in sysUInventoryExchange ArgCount[%d], ArgIdx[%d], ItemID[%d], Count[%d]",
                      argCount, argIdx, itemID, count);
                }
                logRemove.clear();
                logAdd.clear();
                items.clear();
          ...
          int flag =
              sci.getInfo()
                  .apply(
                      getUser(),
                      itemID,
                      getUser().getCharacter(),
                      getUser().getBasicStat(),
                      getUser().getSecondaryStat(),
                      time,
                      false);
  ...
  private void sendMessageAnswer() {
    runningVM
        .getTarget()
        .sendPacket(runningVM.getMsgHistory().get(runningVM.getHistoryPos() - 1).getPacket());
