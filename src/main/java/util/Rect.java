...
public class Rect {
   ...
   * @return <code>true</code> if the specified <code>Point</code> is inside this <code>Rectangle
  ...
  /**
   * public inflate(dx: Number, dy: Number) : Void x -= dx; width += 2 * dx; y -= dy; height += 2 *
   * dy;
   *
   * @param dx
   * @param dy
  ...
  /**
   * Offset the rectangle by adding {@param dx} to its left and right coordinates, and adding
   * {@param dy} to its top and bottom coordinates.
   *
   * @param dx
   * @param dy
   */
  ...
  public boolean intersectRect(Rect rc) {
    if (rc.left < this.left + this.right
        && this.left < rc.left + rc.right
        && rc.top < this.top + this.bottom) return this.top < rc.top + rc.bottom;
    else return false;
  }
