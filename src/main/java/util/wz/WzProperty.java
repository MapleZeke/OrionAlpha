...
public class WzProperty {
      ...
      for (String curPath : path.split("/")) {
        ...
        for (int i = 0; i < list.getLength(); i++) {
          ...
          if (subnode.getNodeType() == Node.ELEMENT_NODE
              && subnode.getAttributes().getNamedItem("name").getNodeValue().equals(curPath)) {
            foundSubNode = true;
            subNode = subnode;
          }
        }
  ...
  public Object getNodeValue() {
    switch (getNodeType()) {
      ...
      case WzVector2D:
        return new Point(
            Integer.parseInt(node.getAttributes().getNamedItem("x").getNodeValue()),
            Integer.parseInt(node.getAttributes().getNamedItem("y").getNodeValue()));
      case WzCanvas:
      case WzConvex2D:
    }
    return null;
