...
/**
 * FileTime Specifically recording time in 100-nanosecond intervals starting from January 1, 1601.
...
public class FileTime {
  ...
  public static final long FILETIME_MILLISECOND = 10000,
  ...
  public static void ftSubtract(
      FileTime from,
      FileTime to,
      Pointer<Integer> day,
      Pointer<Integer> hour,
      Pointer<Integer> min,
      Pointer<Integer> sec) {
    long time =
        ((from.highDateTime - (long) to.highDateTime) << 32) - to.lowDateTime + from.lowDateTime;
    if (day != null) day.set((int) (time / FileTime.FILETIME_DAY));
    if (hour != null) hour.set((int) ((time / FileTime.FILETIME_HOUR) % 24));
    if (min != null) min.set((int) ((time / FileTime.FILETIME_MINUTE) % 60));
    if (sec != null) sec.set((int) ((time / FileTime.FILETIME_SECOND) % 60));
  }
