...
'Script: Furthers a user\'s progress in Bowman 2nd Job Advancement'
...
if self.UserGetJob() == 300 and self.UserGetLevel() >= 30:
    if self.InventoryGetItemCount(4031013) >= 30:
        ...
        ret = self.InventoryExchange(0, 4031013, -nBlack, 4031010, -1, 4031012, 1)
        if not ret:
            self.say("Please, check whether or not you have room in your Etc inventory.")
        else:
            self.RegisterTransferField(106010000, "")
    else:
    ...
    self.sayNext("What are you doing here? This is for bowmans who are ready for advancement.")
    self.RegisterTransferField(106010000, "")
