...
'Script: Furthers a user\'s progress in Magician 2nd Job Advancement'

if self.UserGetJob() == 200 and self.UserGetLevel() >= 30:
    if self.InventoryGetItemCount(4031013) >= 30:
        self.sayNext("Ohhhhh...you collected all 30 Dark Marbles!! It should have been difficult...just incredible! You've passed the test and for that, I'll reward you #b#t4031012##k. Take that and go back to #m101000000#.")
        nBlack = self.InventoryGetItemCount(4031013)
        ret = self.InventoryExchange(0, 4031013, -nBlack, 4031009, -1, 4031012, 1)
        if not ret:
            self.say("Please, check whether or not you have room in your Etc inventory.")
        else:
            self.RegisterTransferField(101020000, "")
    else:
    ...
    self.sayNext("What are you doing here? This is for magicians who are ready for advancement.")
    self.RegisterTransferField(101020000, "")
